...
export default function AddFillUp() {
  ...
  const recalcTotal = () => {
    const l = form.getValues("liters") ?? 0;
    const p = form.getValues("pricePerUnit") ?? 0;
    const d = parseFloat(form.getValues("discount") || "0") || 0;
    const eff = Math.max(0, p - d / 100);
    if (l > 0 && eff > 0) form.setValue("totalCost", parseFloat((l * eff).toFixed(2)));
  };
  ...
  const onSubmit = async (values: FillUpForm) => {
    ...
    if (values.isFull && fillUps.length > 0) {
      ...
      if (lastFull && values.odometer > lastFull.odometer) {
        const distKm = values.odometer - lastFull.odometer;
        fuelEconomy = parseFloat(((values.liters / distKm) * 100).toFixed(2));
      }
    }
    try {
      await createFillUp({
      ...
      setLocation("/fillups");
    } catch { toast.error("Failed to save fill-up"); }
  };

  return (
    <div className="flex flex-col gap-5 page-enter pb-8 pt-4">
      <div className="flex items-center gap-3">
        <button onClick={() => setLocation("/fillups")} className="w-8 h-8 rounded-xl bg-card border border-card-border flex items-center justify-center shrink-0">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4" noValidate>
          ...
          <div className="bg-card border border-card-border rounded-2xl p-4 space-y-4">
            <FormField control={form.control} name="date" render={({ field }) => (
              <FormItem><FormLabel>Date & Time</FormLabel><FormControl><Input type="datetime-local" {...field} className="h-11" /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="odometer" render={({ field }) => (
              <FormItem><FormLabel>Odometer (km)</FormLabel><FormControl><Input type="number" inputMode="decimal" {...field} className="h-12 text-lg font-semibold" /></FormControl><FormMessage /></FormItem>
            )} />
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="number" step="0.01" inputMode="decimal" placeholder="0.00"
                      ...
                      onChange={e => { field.onChange(e.target.value === "" ? "" : parseFloat(e.target.value)); setTimeout(recalcTotal, 0); }}
                      className="h-11" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              ...
              <FormItem>
                ...
                <FormControl>
                  <Input type="number" step="0.1" inputMode="decimal" placeholder="0.0"
                    {...field}
                    onChange={e => { field.onChange(e); setTimeout(recalcTotal, 0); }}
                    className="h-11" />
                </FormControl>
                <FormMessage />
              </FormItem>
            ...
            <FormField control={form.control} name="totalCost" render={({ field }) => (
              <FormItem>
                <FormLabel>Total Cost ({currency}) <span className="text-muted-foreground font-normal text-xs">— auto-calculated</span></FormLabel>
                <FormControl>
          ...
          <div className="bg-card border border-card-border rounded-2xl p-4 space-y-4">
              ...
              <FormItem className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground flex items-center gap-1 mt-0.5">
                    <Info className="h-3 w-3" />Required for accurate fuel economy
                  </p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField control={form.control} name="station" render={({ field }) => (
                <FormItem><FormLabel>Station</FormLabel><FormControl><Input placeholder="e.g. Petro-Canada" {...field} className="h-11" /></FormControl></FormItem>
              )} />
              <FormField control={form.control} name="fuelGrade" render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    ...
                    <SelectContent>
                      {["Regular", "Mid-Grade", "Premium", "Diesel", "E85"].map(g => <SelectItem key={g} value={g}>{g}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </FormItem>
              )} />
            ...
            <FormField control={form.control} name="notes" render={({ field }) => (
              <FormItem><FormLabel>Notes</FormLabel><FormControl><Textarea placeholder="Any extra details…" {...field} className="resize-none" rows={2} /></FormControl></FormItem>
            )} />
          </div>

          <Button type="submit" className="w-full h-13 text-base font-bold rounded-2xl">Save Fill-Up</Button>
        </form>
      </Form>
    </div>
  );
